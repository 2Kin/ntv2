...
class AJAXChatTemplate {
	...
	function replaceTemplateTags($tag, $tagContent) {
		switch($tag) {
			...
			case 'LOGIN_URL':
				return $this->ajaxChat->getConfig('loginData');
	...
	function getStyleSheetLinkTags() {
		...
		foreach($this->ajaxChat->getConfig('styleAvailable') as $style) {
			...
			$styleSheets .= '<link rel="'.$alternate.'stylesheet" type="text/css" href="'.$this->ajaxChat->getChatURL().'css/'.rawurlencode($style).'.css" title="'.$this->ajaxChat->htmlEncode($style).'"/>';
		}
		return $styleSheets;
	}
